...
from Utility.Utils import getObjectDictionary, getObject, getWordTag
...
class PosTaggingRDRTree(RDRTree):
    ...
    def buildTreeFromCorpus(self, startStateFile, correctFile):
        ...
        for startTag in objects:
            print "\n*******\nBuilding rules for the current initialized tag: %s" % startTag
            print "Number of correct tags : %d" % len(objects[startTag])
            print "Correct tags:", objects[startTag].keys()
            ...
            print "Total number of objects with the current initialized tag: %d" % total
